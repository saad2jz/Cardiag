...
function card(garage) {
  const premium = garage.premium?.active ? '<span class="garage-premium-badge">Recommandé</span>' : '';
  return `<article class="garage-card${garage.premium?.active ? ' garage-card-premium' : ''}"><a href="/garages/${encodeURIComponent(garage.slug)}">${premium}<h2>${escapeHtml(garage.name)}</h2><p>${escapeHtml(garage.city)}${garage.postalCode ? ` · ${escapeHtml(garage.postalCode)}` : ''}</p><p>${garage.reviewCount ? `★ ${garage.ratingAverage.toFixed(1)}/5 (${garage.reviewCount} avis)` : 'Pas encore d’avis publié'}</p><p>${escapeHtml((garage.specialties || []).join(' · ') || 'Spécialités à venir')}</p></a></article>`;
}
