...
function emblemMarkup(name) {
  ...
  if (logoPath) {
    return `<span class="brand-emblem has-official-logo" aria-hidden="true"><img src="${logoPath}" alt="" loading="lazy" decoding="async"><span class="brand-logo-fallback">${emblemSvg(name)}</span></span>`;
  }
...
const VISUAL_STEPS = [
  ...
  { id: 'motorisationSelect', key: 'engine', icon: '⚙️', dependsOn: 'generationSelect' },
];
...
function buildVisualStep(config) {
  ...
  panel.innerHTML = `
    ...
    <div class="vehicle-choice-grid" role="listbox" aria-label="${title}"></div>
    <button class="brand-picker-more" type="button"></button>
  `;
  return panel;
}
...
export function initializeBrandPicker(vehicles = []) {
  ...
  function renderVisualStep(state) {
    ...
    gridElement.innerHTML = visible.map((option) => `
      <label class="vehicle-choice-card${targetValue === option.value ? ' is-selected' : ''}" role="option" aria-selected="${targetValue === option.value}" data-value="${option.value.replaceAll('&', '&amp;').replaceAll('"', '&quot;')}">
