...
export async function initializeSyncQueue(){
  ...
  const renderNotice=(state,detail)=>{
    const labels={
      pending:'Synchronisation en attente : connexion indisponible.',
      syncing:'Synchronisation des fiches…',
      synced:'Fiches synchronisées.',
      conflict:'Conflit détecté : les données locales ont été conservées.',
      reauth:'Session expirée : reconnectez-vous pour synchroniser.',
      error:'Synchronisation impossible pour le moment.',
      migrating:'Sauvegarde des fiches locales…',
    };
    if(state==='idle'){syncNotice.hidden=true;return;}
    syncNotice.hidden=false;
    syncNotice.dataset.state=state;
    syncNotice.replaceChildren(document.createTextNode(labels[state]||labels.error));
    if(['pending','reauth','error'].includes(state)){
  ...
  async function drain(){
    if(draining)return;
    if(!window.cardiagConnectivity?.online){publish('pending');return;}
    if(!window.cardiagAuth?.user){publish('reauth');return;}
    draining=true;let conflictCount=0;publish('syncing');
    try{for(const job of await all()){if(job.type==='history'){try{const result=await window.cardiagAuth.api('/api/account/history',{method:'PUT',body:JSON.stringify({records:job.records})});applySyncResult(result);}catch(error){if(error.status!==409||!error.payload)throw error;const applied=applySyncResult(error.payload);conflictCount+=applied.conflicts;}}await remove(job.id)}publish(conflictCount?'conflict':'synced',{conflicts:conflictCount});}
    catch(error){
      const status=Number(error?.status||0);
      publish(status===401||status===403?'reauth':navigator.onLine===false?'pending':'error',{reason:error?.code||error?.message||'unknown'});
